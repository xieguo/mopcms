...
class helper_splitword
{
    ...
    public $addonDicFile = 'data/addonsDic.dic';

    //主词典
    public $dicStr = '';
    public $mainDic = array();
    public $mainDicInfos = array();
    public $mainDicFile = 'data/mainDic.dic';
    ...
    function SetSource( $source, $source_charset='utf-8', $target_charset='utf-8' )
    {
        ...
        if( $source != '' ){
            ...
            }elseif( preg_match("/^gb/", $source_charset) ) {
                $this->sourceString = iconv('utf-8', 'ucs-2', iconv('gb18030', 'utf-8//ignore', $source));
            }elseif( preg_match("/^big/", $source_charset) ) {
    ...
    {
        $dicAddon = MOPDATA.$this->addonDicFile;
        if($maindic=='' || !file_exists(dirname(__FILE__).'/'.$maindic) ){
            $dicWords = MOPDATA.$this->mainDicFile ;
        } else {
    ...
    function OptimizeResult( &$smarr, $spos )
    {
        ...
        for(; $i < $arlen; $i++){
            ...
            //检测前导词(通常是姓)
            elseif( isset( $this->addonDic['n'][ $smarr[$i] ] ) ){
                $is_rs = false;
            ...
            //检测后缀词(地名等)
            elseif( isset($this->addonDic['a'][$nw]) ){
                $is_rs = false;
                //词语是副词或介词不作为前缀
                if( strlen($cw)>2 ){
    ...
    function SourceResultCharset()
    {
        if( preg_match("/^utf/", $this->targetCharSet) ) {
            $rs = 1;
        }elseif( preg_match("/^gb/", $this->targetCharSet) ) {
            $rs = 2;
        }elseif( preg_match("/^big/", $this->targetCharSet) ) {
            $rs = 3;
        }else {
            $rs = 4;
        }
    ...
    function ExportDict( $targetfile )
    {
        ...
        foreach($this->mainDic as $k=>$v){
            ...
            foreach( $v[1] as $wk => $wv){
                ...
                if($arr != ''){
                    fwrite($fp, filterWords($wd.','.$arr['c'].','.$arr['m']."\n"));
                }else{
                    continue;
                }
            }
    ...
    function MakeDict( $sourcefile, $maxWordLen=16, $target='' )
    {
        ...
        foreach($narr as $k=>$v){
            ...
            fwrite($fp, filterWords(pack('n', $alLen)) );
            fwrite($fp, filterWords($allstr));
        }
        fclose($fp);
    }
    ...
    //用户全文搜索，gq用到
    function arr_keys($str){
        $this->SetSource($str, CHARSET, CHARSET);
        $this->StartAnalysis();
        $indexs = (array)$this->GetFinallyIndex();
        $str1 = '';
     ...
     * 非自动拆词：$str必须以,隔开
     */
    function md5_keys($str,$auto=true){
        if($auto){
            $arr = $this->arr_keys($str);
        }else{
            $arr = explode(',', $str);
        }
        $tt = '';
        foreach ($arr as $k=>$v) {
            $tt.=substr(md5($v),0,16)." ";
        }
        return trim($tt,' ');
    }

    function search_keys($str){
        $arr = $this->arr_keys($str);
        $tt = '';
        $i=0;
        foreach ($arr as $k=>$v) {
